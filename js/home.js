...
async function iniciar() {
  ...
  /* ---------- destaques em círculo (atalho para cada categoria) ---------- */
  const CORES = [
    ["var(--terra)", "#fffaf2"], ["var(--rosa)", "#fffaf2"], ["var(--oliva)", "#fffaf2"],
    ["var(--bege)", "#5b4327"], ["var(--dourado)", "#5b4327"],
  ];
  $("#destaques").innerHTML = categorias
    .map((c, i) => {
      const [cor, tinta] = CORES[i % CORES.length];
      return `<a class="story" href="categoria.html?c=${esc(c.slug)}">
        <span class="story__ring" style="--c:${cor};--ink:${tinta}">${iconeCategoria(c.slug)}</span>
        <span class="story__label">${esc(c.nome)}</span></a>`;
    })
    .join("");
...
/** Escolhe o ícone pelo nome da categoria; se não reconhecer, usa o cabide. */
function iconeCategoria(slug) {
  const mapa = [
    [/vest/, ico.vestido],
    [/blus|camis|top|cropped|regata/, ico.blusa],
    [/short|bermuda/, ico.short],
    [/calc|jeans|legging/, ico.calca],
    [/conjunt|biquin|praia|look/, ico.conjunto],
    [/acess|bolsa|bijou/, ico.sacola],
  ];
  return (mapa.find(([re]) => re.test(slug)) || [, ico.cabide])[1];
}

/** Deixa a última palavra do título em itálico e terracota — dá ritmo à manchete. */
